...
if "C9_PID" in os.environ:
    print """detected Cloud9 IDE"
        We will need to remove the .pydistutils file
        This is harmless"""
    goahead = raw_input("should we go ahead? [Y/N] ")
    if goahead in ['y','Y']:
        print "Great, we're going ahead"
        success = subprocess.call([
           "rm", 
           "%(HOME)s/.pydistutils.cfg" % os.environ
           ])
    else:
        print "Stopping now"
        sys.exit()
